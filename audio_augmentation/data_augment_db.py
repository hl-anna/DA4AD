...
def id_split(frac=10, print_info=False, cv=True):
    ...
    with open('./AD_train_ids.txt', "r") as clf:
        lines = clf.readlines()
    ids_ad = [re.sub('.cha\n', 'c-en', line) for line in lines]

    # Load the ids of Control subjects into a list
    with open('./HC_train_ids.txt', "r") as clf:
        lines = clf.readlines()
    ids_hc = [re.sub('.cha\n', 'c-en', line) for line in lines]
...
def segment_test_data(use_autovc, test_path, logmel=True, duration=16, augment=False):
    ...
    with open(f'{PATH}/test_db_labels.txt', "r") as clf:
        lines = clf.readlines()
    ids_and_labels = [line.strip() for line in lines]
    ids = [item[:9] for item in ids_and_labels]

    test_ids_ad, test_ids_hc = [], []

    for id in ids_and_labels:
        if int(id[-1]) == 1:
            test_ids_ad.append(id[:9])
        elif int(id[-1]) == 0:
            test_ids_hc.append(id[:9])
...
file_lst2_ad = os.listdir("./ad_audio_db_vc/")
file_lst2_hc = os.listdir("./hc_audio_db_vc/")
...
for i in tqdm(range(len(train_data))):
    ...
    file_path = './data_db_' + augmentation_type + '/' + str(i) + '.csv'
    np.savetxt(file_path, file, delimiter=',')
    
np.savetxt('./train_labels_db_' + augmentation_type, train_labels, delimiter=',')
...
class Dataset_AST(Dataset):
    ...
    def __init__(self, aug_type):
        self.data_dir = "./data_db_" + aug_type
        ...
        if aug_type=="test":
            self.label_dir = "./test_segment_db_labels"
        else:
            self.label_dir = "./train_labels_db_" + aug_type
...
test_sample_labels = pd.read_csv("./test_db_labels.txt", sep=',', header=None)
test_sample_labels = test_sample_labels.iloc[:,1]
test_seg_id_a = pd.read_csv("test_seg_id_a_db.csv")
...
def segment_to_test(y_pred, y_pred_proba, test_seg_id_a, test_sample_labels):
    ...
    acc = sum(test_sample_labels==pred_labels)/111
...
ad_path = "./ad_audio_db"
hc_path = "./hc_audio_db"
test_path = "./test_audio_db"
use_autovc = False
use_augment = True
augmentation_type = 'specaugment' #["masking", "shift", "loudness", "noise", "pitch", "vtlp", "speed", "normalisation", "specaugment"]
random_aug_size = 1

print('AUGMENTATION TYPE IS:  ', augmentation_type)
labels_all = pd.read_csv('train_labels_db_' + augmentation_type, header=None)
...
for i in range(1,6):
    ...
    print(f'Model accuracy on test set: {score_acc, score_f1}')
    pd.DataFrame(y_pred_sample).to_csv("./ast_test_preds_db/" + augmentation_type + '_' + str(i) + '.csv')

pd.DataFrame(acc_results).to_csv("./ast_test_preds_db/" + augmentation_type + '_acc.csv')
